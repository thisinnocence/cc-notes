// Simplified PV emulator framework
// Follows the user's naming and Google-like style rules (UpperCamel for
// classes/functions, snake_case_ for members, kConstantCase for constants).
...
// Interrupt controller: manages pending (IP) and enable (IE) bits for multiple
// IRQs
 ...
 public:
  ...
  void Step() {
    ...
    if (maybe_irq != -1) {
      ...
      if (trace_)
        std::cout << "[CPU] IRQ " << irq << " taken at pc=" << pc_ << '\n';
      // 保存 PC 到栈
      Push32(pc_);
    ...
    switch (opcode) {
      ...
      case kOpJmp: {
        uint32_t addr = mem_.Read32(pc_ + 1);
        if (trace_)
          std::cout << "[CPU] EXEC JMP pc=" << pc_ << " -> " << addr << '\n';
        pc_ = addr;
        break;
      }
      case kOpPrint: {
        ...
        if (trace_)
          std::cout << "[CPU] EXEC PRINT pc=" << pc_ << " addr=" << addr
                    << '\n';
        pc_ += 1 + 4;
        break;
      }
      case kOpIret: {
        ...
        uint32_t ret = Pop32();
        if (trace_)
          std::cout << "[CPU] EXEC IRET returning to pc=" << ret << '\n';
        pc_ = ret;
        break;
      }
      case kOpHalt: {
 ...
 public:
  ...
  void LoadProgram(const std::vector<uint8_t>& image, uint32_t load_addr) {
    for (size_t i = 0; i < image.size(); ++i)
      mem_.WriteByte(load_addr + i, image[i]);
  }
...
int main() {
  ...
  // imm 123
  for (int i = 0; i < 4; ++i)
    image[p++] = static_cast<uint8_t>((123 >> (8 * i)) & 0xFF);
  image[p++] = kOpStore;
  uint32_t store_addr = 200;
  for (int i = 0; i < 4; ++i)
    image[p++] = static_cast<uint8_t>((store_addr >> (8 * i)) & 0xFF);
  image[p++] = kOpIret;
  ...
  image[q++] = kOpLoadImm;
  for (int i = 0; i < 4; ++i)
    image[q++] = static_cast<uint8_t>((10 >> (8 * i)) & 0xFF);
  // ADD_IMM 5
  image[q++] = kOpAddImm;
  for (int i = 0; i < 4; ++i)
    image[q++] = static_cast<uint8_t>((5 >> (8 * i)) & 0xFF);
  // SUB_IMM 3
  image[q++] = kOpSubImm;
  for (int i = 0; i < 4; ++i)
    image[q++] = static_cast<uint8_t>((3 >> (8 * i)) & 0xFF);
  // STORE to store_addr
  image[q++] = kOpStore;
  for (int i = 0; i < 4; ++i)
    image[q++] = static_cast<uint8_t>((store_addr >> (8 * i)) & 0xFF);
  ...
  image[q++] = kOpJmp;
  for (int i = 0; i < 4; ++i)
    image[q++] = static_cast<uint8_t>((halt_addr >> (8 * i)) & 0xFF);
  // HALT
  image[q++] = kOpHalt;
  ...
  std::cout
      << "[Main] Starting emulator. Timer will request IRQ 1 every 5 ticks.\n";
  emu.Run(200);
