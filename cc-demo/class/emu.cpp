  ...
  void Step() {
    ...
    switch (opcode) {
      ...
      case kOpLoadImm: {
        ...
        acc_ = imm;
        if (trace_)
          std::cout << "[CPU] EXEC LOAD_IMM pc=" << pc_ << " imm=" << imm << " -> acc=" << acc_
                    << '\n';
        pc_ += 1 + 4;
        break;
      }
      case kOpStore: {
        ...
        if (trace_)
          std::cout << "[CPU] EXEC STORE pc=" << pc_ << " addr=" << addr << " value=" << acc_
                    << '\n';
        pc_ += 1 + 4;
        break;
      }
      case kOpAddImm: {
        ...
        acc_ += imm;
        if (trace_)
          std::cout << "[CPU] EXEC ADD_IMM pc=" << pc_ << " imm=" << imm << " -> acc=" << acc_
                    << '\n';
        pc_ += 1 + 4;
        break;
      }
      case kOpSubImm: {
        ...
        acc_ -= imm;
        if (trace_)
          std::cout << "[CPU] EXEC SUB_IMM pc=" << pc_ << " imm=" << imm << " -> acc=" << acc_
                    << '\n';
        pc_ += 1 + 4;
        break;
      }
      ...
      default: {
        std::cerr << "[CPU] Unknown opcode 0x" << std::hex << int(opcode) << " at pc=" << std::dec
                  << pc_ << '\n';
        halted_ = true;
        break;
      }
